/// <summary>
/// Report A01 BOA Check (50040)
/// </summary>
report 50040 "A01 BOA Check"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Source/Report/Layout/BOACheck.rdl';
    Caption = 'Chèque BOA';
    ...
    dataset
    {
        ...
        dataitem(GenJnlLine; "Gen. Journal Line")
        {
            ...
            dataitem(CheckPages; "Integer")
            {
                ...
                column(CompanyPicture; RespCenter."A01 Logo")
                {
                }
                column(Head001Lbl; Head001Lbl)
                {
                }
                column(Head002Lbl; Head002Lbl)
                {
                }
                column(Head003Lbl; Head003Lbl)
                {
                }
                column(CheckNumbLbl; CheckNumbLbl)
                {
                }
                column(NonEndossableLbl; NonEndossableLbl)
                {
                }
                column(ArLbl; ArLbl)
                {
                }
                column(ARedigerExcluLbl; ARedigerExcluLbl)
                {
                }
                column(PayezContreLbl; PayezContreLbl)
                {
                }
                column(SommeEnLettreLbl; SommeEnLettreLbl)
                {
                }
                column(ALordreDeLbl; ALordreDeLbl)
                {
                }
                column(PayableALbl; PayableALbl)
                {
                }
                ...
                column(NepasEcrireLbl; NepasEcrireLbl)
                {
                }
                column(LettrePaiementLbl; LettrePaiementLbl)
                {
                }
                column(NomBeneficiaireLbl; NomBeneficiaireLbl)
                {
                }
                column(AdresseBeneficiaireLbl; AdresseBeneficiaireLbl)
                {
                }
                column(DateLbl; DateLbl)
                {
                }
                column(VeuillezAccuserLbl; VeuillezAccuserLbl)
                {
                }
                column(NomsPrenomsLbl; NomsPrenomsLbl)
                {
                }
                column(CNILbl; CNILbl)
                {
                }
                column(DuplicataLbl; DuplicataLbl)
                {
                }
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    field(BankAccount; BankAcc2."No.")
                    {
                        ...
                        ToolTip = 'Spécifie le compte bancaire à partir duquel les chèques imprimés sont tirés.';
                    ...
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'N° dern. chèque';
                        ToolTip = 'Spécifie la valeur du champ N° dern. chèque sur la fiche compte bancaire.';
                    }
                    field(OneCheckPerVendorPerDocumentNo; OneCheckPrVendor)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Un chèque par fournisseur par n° document';
                        MultiLine = true;
                        ToolTip = 'Indique si un seul chèque est imprimé par fournisseur et par numéro de document.';
                    }
                    field(ReprintCheck; ReprintChecks)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Réimprimer les chèques';
                        ToolTip = 'Indique si les chèques sont imprimés à nouveau si vous avez annulé l''impression suite à un problème.';
                    }
                    field(TestPrinting; TestPrint)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Impression test';
                        ToolTip = 'Indique si vous souhaitez imprimer les chèques sur papier blanc avant de les imprimer sous forme de chèque.';
                    }
                    field(PreprintedStubs; PreprintedStub)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Formulaire préimprimé';
                        ToolTip = 'Indique si vous utilisez les formulaires chèque avec souches pré-imprimées.';
                    }
                }
            }
        }
    ...
    var
        ...
        Head001Lbl: Label 'Siège social : Rue du Dr Joseph RASETA, Andranomahery . BP 3806, antananarivo 101';
        Head002Lbl: Label 'Tél. 23 277 11 / 23 692 11 . E-mail : star@.mg - www.star.mg';
        Head003Lbl: Label 'STAR S.A au capital de Ar 4 290 000 000 . N° Ident. 11041 11 1951 0 10008 . RCS Antananarivo 2002 B 00517 . NIF 2000 023 089';
        CheckNumbLbl: Label 'Chèque N°';
        NonEndossableLbl: Label 'NON ENDOSSABLE';
        ArLbl: Label 'Ar';
        ARedigerExcluLbl: Label 'A rédiger exclusivement en Ariary';
        PayezContreLbl: Label 'Payez contre ce chèque :';
        SommeEnLettreLbl: Label '(Somme en toutes lettres)';
        ALordreDeLbl: Label 'A  l''ordre  de';
        PayableALbl: Label 'Payable à ';
        SignatureLbl: Label 'Signature';
        LeLbl: Label 'le';
        NepasEcrireLbl: Label 'Ne pas écrire en dessous de cette zone';
        LettrePaiementLbl: Label 'LETTRE DE PAIEMENT PAR CHEQUE N° ';
        NomBeneficiaireLbl: Label 'Nom bénéficiaire :';
        AdresseBeneficiaireLbl: Label 'Adresse :';
        DateLbl: Label 'Date :';
        VeuillezAccuserLbl: Label 'Veuillez accuser réception du chèque N° ';
        NomsPrenomsLbl: Label 'Noms et prénoms :';
        CNILbl: Label 'N° CNI :';
        DuplicataLbl: Label 'Duplicata :';

        Text000: Label 'L''aperçu n''est pas autorisée';
        Text001: Label 'Le N° dern. chèque doit être rempli.';
        Text002: Label 'Les filtres sur %1 et %2 ne sont pas autorisés.';
        Text003: Label 'XXXXXXXXXXXXXXXX';
        Text004: Label 'doit être renseigné.';
        Text005: Label 'Le compte bancaire et la ligne du journal général doivent avoir la même devise.';
        Text008: Label 'Les deux comptes bancaires doivent avoir la même devise.';
        Text010: Label 'XXXXXXXXXX';
        Text011: Label 'XXXX';
        Text012: Label 'XX.XXXXXXXXXX.XXXX';
        Text013: Label '%1 existe déjà.';
        Text014: Label 'Vérifier si %1 %2';
        Text016: Label 'Dans la fenêtre d''impression, l''option Un chèque par fournisseur par n° document ne doit pas être activée lorsque le champ ID de lettrage est spécifié dans les lignes du journal.';
        Text019: Label 'Total';
        Text020: Label 'Le montant total du chèque %1 est %2. Le montant doit être positif.';
        Text021: Label 'ANNULER ANNULER ANNULER ANNULER ANNULER ANNULER ANNULER ANNULER ANNULER ANNULER ANNULER ANNULER ANNULER ANNULER ANNULER ANNULER';
        Text022: Label 'NON-NEGOCIABLE';
        Text023: Label 'Impression test';
        Text024: Label 'XXXX.XX';
        Text025: Label 'XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX';
        Text026: Label 'ZERO';
        Text027: Label 'CENT';
        Text028: Label 'ET';
        Text029: Label '%1 Resulte d''un nombre écrit qui est trop long.';
        Text030: Label ' est déjà lettré à %1 %2 pour le client %3.';
        Text031: Label ' est déjà lettré à %1 %2 pour le fournisseur %3.';
        Text032: Label 'UN';  // ONE
        Text033: Label 'DEUX';
        Text034: Label 'TROIS';
        ...
        Text057: Label 'QUATRE-VINGTS';
        Text058: Label 'QUATRE-VINGT-DIX';
        Text059: Label 'MILLE';
        Text060: Label 'MILLION';
        Text061: Label 'MILLIARD';
        Text063: Label 'Net Amount %1';
        Text064: Label '%1 ne doit pas être %2 pour %3 %4.';
        Text065: Label 'Sous-total';
        Text10800: Label 'ARIARY';
        Text10801: Label 'CENT';
        CheckNoTextCaptionLbl: Label 'N° chèque';
        LineAmountCaptionLbl: Label 'Montant net';
        LineDiscountCaptionLbl: Label 'Remise';
        AmountCaptionLbl: Label 'Montant';
        DocNoCaptionLbl: Label 'N° document';
        DocDateCaptionLbl: Label 'Date Document';
        CurrencyCodeCaptionLbl: Label 'Code devise';
        YourDocNoCaptionLbl: Label 'Votre n° de document.';
        TransportCaptionLbl: Label 'Transport';
        BlockedEmplForCheckErr: Label 'Vous ne pouvez pas imprimer de chèque car l''employé %1 est bloqué pour des raisons de confidentialité.', Comment = '%1 - N° de l''employé';
        AlreadyAppliedToEmployeeErr: Label ' est déjà lettré à %1 %2 pour l''employé %3.', Comment = '%1 = Type Document, %2 = N° Document., %3 = N° de l''employé';
    ...
    local procedure ChangeNomBeneficiaire(var Benef: Text[50]; GJnlLine2: Record "Gen. Journal Line")
    var
        CustomerBankAccount: Record "Customer Bank Account";
        VendorBankAccount: Record "Vendor Bank Account";
    begin

        if GJnlLine2."Account Type" = GJnlLine2."Account Type"::Customer then begin
            if CustomerBankAccount.Get(GJnlLine2."Account No.", GJnlLine2."Recipient Bank Account") then
                if CustomerBankAccount."Name 2" <> '' then
                    Benef := CustomerBankAccount."Name 2";
        end;

        if GJnlLine2."Account Type" = GJnlLine2."Account Type"::Vendor then begin
            if VendorBankAccount.Get(GJnlLine2."Account No.", GJnlLine2."Recipient Bank Account") then
