/// <summary>
/// Report A01 Credit Memo (ID 50038).
/// </summary>
report 50038 "A01 CreditMemo"
{
    Caption = 'Avoir vente';
        ...
        dataitem(Header; "Sales Cr.Memo Header")
        {
            ...
            dataitem(Line; "Sales Cr.Memo Line")
            {
                ...
                begin
                    ...
                    A01LineQty := Round(Line.Quantity, 0.001, '<');
            ...
            dataitem(Totals; "Integer")
            {
                ...
                begin
                    ...
                    AfkFormattedTotalVAT := Format(TotalAmountVAT, 0, AutoFormat.ResolveAutoFormat("Auto Format"::AmountFormat, Header."Currency Code"));
                    AfkFormattedTotalHT := Format(TotalAmount, 0, AutoFormat.ResolveAutoFormat("Auto Format"::AmountFormat, Header."Currency Code"));
                    AfkFormattedTotalTTC := Format(TotalAmountExclInclVATValue, 0, AutoFormat.ResolveAutoFormat("Auto Format"::AmountFormat, Header."Currency Code"));

                    AfkTotalAmountInclVAT_LCY := CurrencyExchangeRate.ExchangeAmtFCYToLCY(Header."Posting Date", Header."Currency Code", TotalAmountExclInclVATValue, Header."Currency Factor");
                    AfkTotalAmount_LCY := CurrencyExchangeRate.ExchangeAmtFCYToLCY(Header."Posting Date", Header."Currency Code", TotalAmount, Header."Currency Factor");
                    AfkTotalVAT_LCY := CurrencyExchangeRate.ExchangeAmtFCYToLCY(Header."Posting Date", Header."Currency Code", TotalAmountVAT, Header."Currency Factor");
                    ...
                    AfkTotalAmountInclVAT_LCYText := Format(AfkTotalAmountInclVAT_LCY, 0, AutoFormat.ResolveAutoFormat("Auto Format"::AmountFormat, AfkLocalCurrency.Code));
                    AfkTotalAmountInclVAT_LCYText := Format(AfkTotalAmountInclVAT_LCY, 0, '<Precision,2><Standard Format,0>');
                    AfkTotalAmount_LCYText := Format(AfkTotalAmount_LCY, 0, AutoFormat.ResolveAutoFormat("Auto Format"::AmountFormat, AfkLocalCurrency.Code));
                    AfkTotalAmount_LCYText := Format(AfkTotalAmount_LCY, 0, '<Precision,2><Standard Format,0>');
                    AfkTotalVAT_LCYText := Format(AfkTotalVAT_LCY, 0, AutoFormat.ResolveAutoFormat("Auto Format"::AmountFormat, AfkLocalCurrency.Code));
    ...
    var
        ...
        SalesInvoiceLbl: Label 'AVOIR';
        InvoiceDateLbl: Label 'Date de facture :';
        DateofprintLbl: Label 'Date d''impression :';
        VendorNameLbl: Label 'Nom du vendeur :';
        VendorMatriculeLbl: Label 'Matricule du vendeur :';
        UnitNameLbl: Label 'Nom de l''unité :';
        UnitAddressLbl: Label 'Adresse de l''unité :';
        UnitCityLbl: Label 'Ville';
        UnitPostalCodeLbl: Label 'Code postal :';
        CustomerNameLbl: Label 'Nom du client :';
        CustomerAddressLbl: Label 'Adresse du client :';
        NIFLbl: Label 'NIF :';
        STATLbl: Label 'STAT :';
        RCSLbl: Label 'RCS :';
        CustomerPhoneLbl: Label 'Téléphone :';
        ProductCodeLbl: Label 'Code';
        DesignationLbl: Label 'Designation';
        QuantityLbl: Label 'Quantité';
        UnitPriceLbl: Label 'Prix unitaire (HT)';
        DiscountPercentLbl: Label 'Remise';
        DiscountAmountLbl: Label 'Montant (HT)';
        HTPriceLbl: Label 'TOTAL HT';
        VAT20Lbl: Label 'TVA';
        TTCPriceLbl: Label 'TOTAL TTC';
        ArrestedOfSumLbl: Label 'MONTANT TTC :';
        ClientSignLbl: Label 'Signature du client';
        CompanySignLbl: Label 'Signature de l''entrprise';
